...
- **类型标签**：每家店挂 0~6 个类型标签（杭帮菜 / 面 / 小吃 / 火锅 / 烧烤 / 夜宵 / 换口味…），首页顶部俏皮话 chips 一键切换；一家店可同时有多个标签
- **两个池子**：默认「全部」只抽**精选池**（59 家手工筛过的）；「随便吃点」进**拓展池**（378 家，高德按商圈拉取、评分 ≥4.2）；类型标签两边都含
- **「附近」独立屏**：自动定位 → 半径 1km/1.5km/3km → 店卡列表（⭐评分、¥人均、距你 Xm，可按距离/评分排序浏览）→「就近抽一个」**纯随机**抽取（评分只做入库门槛与展示，不参与抽取）
...
| `GET /api/options` | 公开 | 全量店铺，每条含 `tags: [标签]` 与可选的 `latitude` / `longitude` / `address` / `rating` / `cost`；`?tag=<标签>` 可按类型过滤；`?near=<经度,纬度>&radius=<米>`（默认 1500、上限 10000）按距离过滤并返回 `distance_meters`，只含已定位的店 |
...
- **2026-09-08 · v0.3.2** — 三选一重构为抽取模式:chip 与标签同级互斥,点击只切换抽取方式;「开始选择」跑完抽奖动画后揭晓三张候选卡(名称/类型/人均,三等分铺满一行不裁切),选一张才进单店展示(缩略图/地图)并计入历史。附带:静态资源一律 `Cache-Control: no-cache`(根治"改了没生效"的启发式缓存问题);「开始选择」按钮去动画、配色微调(回到原色系整体提亮一档);「乌萨奇精选」chip 渲染回归修复
- **2026-09-07 · v0.3** — 版本正式定为 v0.3。本版本累计:多类型标签与双池(精选/拓展)、「附近」独立屏与就近抽签、结果页位置缩略图、人均与评分体系、管理入口隐藏、抽签历史与三选一(暂缓)

- **2026-09-06（三选一 + 抽签历史）** — 首页新增「🎲 三选一」：连抽 3 张候选卡（只给名称/类型/人均，不上图），点最心动的一张才进结果屏看地图/缩略图；「今日已选」统计可点开，弹窗列出今天每次抽中的时间与店名（存本机）；两处抽取都计入当日统计

- **2026-09-06（emoji 回填 + 池子清理）** — fetch-pois 新增两个模式：`--refresh-ratings` 顺带给仍为 🍽️ 占位的拓展店按高德分类配 emoji；`--prune` 全量扫一遍商圈删除扫不到的店（闭店/除名），本轮清 25 家。拓展池 349、总池 408→383；🍽️ 占位 206→41；未定位提示文案改访客视角；sw v12

- **2026-09-06（chips 精简 + 定位降级）** — 「狐狸精选」改名「乌萨奇精选」；去掉「随便吃点/垫垫肚子/深夜食堂」三个 chip（店仍在「全部」与数据里，标签保留）；附近屏定位失败/不支持时自动回首页，提示直接从全部里抽

- **2026-09-06（精选降级为标签）** — 「全部」改回 437 家全量直抽；原精选 59 家打上「精选」标签（chip「狐狸精选」），与「随便吃点」等同为池标记，固定排在类型标签前；首页统计改显全量数

- **2026-09-06（附近屏 + 评分入库）** — 就近抽签独立成第四屏「附近」：自动定位 → 半径 1km/1.5km/3km → 店卡列表（⭐评分、¥人均、距你 Xm，浏览可按距离/评分排序，点卡片跳地图）→「就近抽一个」**纯随机**抽取；评分/人均只做入库门槛与展示、不参与抽取。`006` 迁移加 `rating` / `cost` 字段；fetch-pois 入库带评分并新增 `--refresh-ratings` 回填模式（顺带报告可能闭店的店）；sw v8

- **2026-09-06（精选/拓展双池）** — 「全部」默认只抽精选池（59 家）；批量店挂「就近随便吃」标签构成拓展池，点「随便吃点」chip 才进；类型标签筛选两边都含（找火锅时精选和拓展一起出）；「📍 附近」就近抽两个池都算；首页统计与「全部」chip 计数改显精选数
